...
export function Experience() {
  ...
  useGSAP(() => {
    // Animate only the connector lines (not the dots)
    gsap.from('.exp-connector', {
      scaleY: 0,
      transformOrigin: 'top',
      ease: 'none',
      stagger: 0,
  ...
  return (
    <section ref={containerRef} id="experience" className="py-24 px-8 md:px-16 lg:px-24">
      ...
      <div className="flex gap-8">
        {/* Timeline spine: dots + connectors as separate elements */}
          ...
          {EXPERIENCE.map((_, i) => (
            <div key={i} className="flex flex-col items-center">
              ...
              {i < EXPERIENCE.length - 1 && (
                <div className="exp-connector w-px min-h-[90px] bg-gradient-to-b from-[var(--color-accent)]/35 to-[var(--color-accent)]/05" />
              )}
            </div>
          ))}
        </div>
